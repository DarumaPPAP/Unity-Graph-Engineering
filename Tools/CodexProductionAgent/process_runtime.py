#!/usr/bin/env python3
"""Cross-platform subprocess runtime for production smoke execution.

The runtime preserves partial stdout/stderr, forces UTF-8, and terminates the whole
process group/tree on timeout so a timed-out Codex invocation cannot poison later cases.
"""
...
def run_streaming_process(
    ...
    *,
    ...
    stdout_thread = threading.Thread(
        target=pump, args=(process.stdout, stdout_path, stdout_lines, True), daemon=True
    )
    stderr_thread = threading.Thread(
        target=pump, args=(process.stderr, stderr_path, stderr_lines, False), daemon=True
    )
    stdout_thread.start()
    stderr_thread.start()
    ...
    return StreamingProcessResult(
        ...
        process_tree_cleanup=cleanup_status,
