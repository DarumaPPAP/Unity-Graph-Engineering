...
def _profile(request: dict[str, Any]) -> str:
    ...
    if profile not in PROFILES:
        raise OrchestrationError("invalid_profile", f"execution_profile must be one of {sorted(PROFILES)}", "invalid_request")
    return profile
...
def _validate_control_state(state: dict[str, Any], profile: str) -> None:
    ...
    if not isinstance(state.get("todos"), list):
        raise OrchestrationError("incomplete_control_state", "execution_state.todos must be an explicit array")
    _bool(state.get("todos_truncated"), "execution_state.todos_truncated")
    if not isinstance(state.get("available_capabilities"), list):
        raise OrchestrationError("incomplete_control_state", "execution_state.available_capabilities must be an explicit array")
...
def _run_json(command: list[str], payload: dict[str, Any] | None, *, timeout: int, expected_identity=None) -> dict[str, Any]:
    if timeout <= 0 or not command:
        raise OrchestrationError("invalid_controller_invocation", "controller command/timeout is invalid", "invalid_request")
    try:
    ...
    try:
        ...
        raise OrchestrationError("controller_non_json", f"controller returned non-JSON output (exit {completed.returncode})") from exc
    ...
    if expected_identity is not None:
        ...
        if result.get(key) != expected:
            raise OrchestrationError("controller_identity_mismatch", f"expected {key}={expected}, got {result.get(key)!r}")
...
def _continuation(operation: str, state: dict[str, Any], *, now: str | None = None, slots: int | None = None) -> dict[str, Any]:
    ...
    return _run_json(command, state, timeout=DEFAULT_CONTROLLER_TIMEOUT, expected_identity=("controller", "native_continuation"))


def _memory(workspace: Path, request: dict[str, Any]) -> dict[str, Any]:
    return _run_json(
        [sys.executable, str(MEMORY_CONTROLLER), "--workspace-root", str(workspace), "--request", "-"],
        request,
...
def _ix(workspace: Path, request: dict[str, Any]) -> dict[str, Any]:
    ...
    if operation not in IX_OPERATIONS:
        raise OrchestrationError("invalid_ix_operation", f"code_intelligence.operation must be one of {sorted(IX_OPERATIONS)}", "invalid_request")
    ...
    if operation == "trace":
        ...
        if not 1 <= depth <= 5 or not 1 <= cap <= 200:
            raise OrchestrationError("invalid_trace_bounds", "trace depth/cap exceeds orchestrator bounds", "invalid_request")
    ...
    return _run_json(command, None, timeout=DEFAULT_IX_TIMEOUT, expected_identity=("provider", "ix"))


def _ticket(*, profile: str, work_kind: str, state: dict[str, Any], decision: dict[str, Any], source_verification: dict[str, Any]) -> dict[str, Any]:
...
def _source_verification(workspace: Path, profile: str, work_kind: str, request: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    source = _dict(request.get("source_verification", {}), "source_verification")
    completed = bool(source.get("completed", False))
    scope = str(source.get("scope_class", "project_internal" if profile == "personal_full_control" else "portable_artifact"))
    ...
    if not isinstance(paths_raw, list) or not isinstance(evidence_raw, list):
        raise OrchestrationError("invalid_request", "source_verification paths/evidence_refs must be arrays", "invalid_request")
    ...
    if profile != "personal_full_control" and scope not in SAFE_NON_PERSONAL_SCOPES:
        raise OrchestrationError("source_scope_forbidden", f"{profile} may verify only safe source scopes")

    normalized_paths: list[str] = []
    if completed:
        if not paths:
            raise OrchestrationError("source_verification_missing_paths", "completed source verification requires an explicit path")
        for value in paths:
            normalized_paths.append(
                _workspace_relative(
                    workspace,
                    _path_under_workspace(workspace, value, "source_verification.paths[]", require_file=True),
                )
            )
    ...
    if required and completed and not evidence_refs:
        raise OrchestrationError("source_verification_evidence_missing", "mutation source verification requires an evidence_ref")
...
def prepare(workspace: Path, request: dict[str, Any]) -> dict[str, Any]:
    ...
    if decision.get("status") != "ok":
        return _envelope("prepare", "blocked", ready=False, decision=decision, diagnostics=[{"code": "continuation_rejected", "message": "Continuation controller rejected the state."}])
    if not decision.get("should_run", False):
        return _envelope("prepare", "ok", ready=False, decision=decision, required_next_action=decision.get("effective_action", "none"))

    selected = decision.get("selected_todo")
    if not isinstance(selected, dict) or not str(selected.get("id", "")).strip():
        return _envelope(
            "prepare", "ok", ready=False, decision=decision,
            required_next_action=decision.get("effective_action", "materialize_advancement_todo_or_blocker"),
            diagnostics=[{"code": "selected_todo_required_for_ticket", "message": "A concrete selected todo is required before ticket issuance."}],
        )
    ...
    if worker["multiple_workers"]:
        ...
        if claim_projection.get("status") != "ok":
            return _envelope("prepare", "blocked", ready=False, decision=decision, claim=claim_projection, required_next_action="resolve_claim_failure")
        existing = selected.get("lease") or {}
        projected = (claim_projection.get("claim") or {}).get("lease") or {}
        claim_confirmed = bool(existing) and existing.get("id") == projected.get("id") and existing.get("owner") == worker["id"]
        if not claim_confirmed:
            return _envelope(
                "prepare", "ok", ready=False, decision=decision, claim=claim_projection,
                required_next_action="write_claim_to_authoritative_state",
                required_state_writeback={"todo_id": claim_projection.get("todo_id"), "claim": claim_projection.get("claim")},
            )
    ...
    if bool(memory_request.get("enabled", False)):
        ...
        if not query:
            diagnostics.append({"code": "memory_query_empty", "message": "Broad empty-query memory loading is forbidden; projection skipped."})
        else:
            memory_projection = _memory(workspace, {
                "operation": "project",
                "execution_profile": profile,
                "query": query,
            ...
            if memory_projection.get("status") != "ok":
                diagnostics.append({"code": "memory_unavailable", "message": "Memory projection failed; continuing without memory context."})
                memory_projection = None
            else:
                ...
                if profile != "personal_full_control" and any(
                    item.get("scope_class") not in SAFE_NON_PERSONAL_SCOPES for item in data.get("items", [])
                ):
    ...
    if bool(ix_request.get("enabled", False)):
        if profile != "personal_full_control":
            diagnostics.append({"code": "ix_prohibited_for_profile", "message": f"Ix was not invoked for {profile}."})
        else:
            ix_result = _ix(workspace, ix_request)
            if ix_result.get("status") != "ok":
                diagnostics.append({"code": "ix_fallback", "message": "Ix failed/unavailable; use targeted_source_read."})
            elif ix_result.get("low_confidence"):
                diagnostics.append({"code": "ix_low_confidence", "message": "Ix may narrow navigation only."})

    source_ready, source_projection = _source_verification(workspace, profile, work_kind, request)
    if not source_ready:
        return _envelope(
            "prepare", "ok", ready=False, decision=decision, memory=memory_projection,
            code_intelligence=ix_result, source_verification=source_projection,
            required_next_action="direct_source_read", diagnostics=diagnostics,
        )

    ticket = _ticket(profile=profile, work_kind=work_kind, state=state, decision=decision, source_verification=source_projection)
    return _envelope(
        "prepare", "ok", ready=True, decision=decision, memory=memory_projection,
        code_intelligence=ix_result, source_verification=source_projection, ticket=ticket,
        required_next_action="execute_one_bounded_slice", diagnostics=diagnostics,
    )
...
def finalize(workspace: Path, request: dict[str, Any]) -> dict[str, Any]:
    ...
    if ticket.get("execution_profile") != profile:
        raise OrchestrationError("ticket_profile_mismatch", "ticket profile differs from finalize request")
    if ticket.get("goal_id") != state.get("goal_id"):
        raise OrchestrationError("ticket_goal_mismatch", "ticket goal differs from authoritative state")
    ...
    if ticket.get("selected_todo_id") != todo_id:
        raise OrchestrationError("ticket_todo_mismatch", "slice todo differs from prepared ticket")
    ...
    evidence_path = _path_under_workspace(
        workspace, _text(result.get("evidence_file"), "slice_result.evidence_file"),
        "slice_result.evidence_file", require_file=True,
    )
    scope = str(result.get("scope_class", "project_internal" if profile == "personal_full_control" else "portable_artifact"))
    if profile != "personal_full_control" and scope not in SAFE_NON_PERSONAL_SCOPES:
    ...
    if capture.get("status") != "ok":
        return _envelope("finalize", "blocked", ready=False, evidence=capture, required_next_action="resolve_evidence_capture")
    ...
    if bool(atom.get("enabled", False)):
        ...
        atom_projection = _memory(workspace, {
            "operation": "create_atom", "memory_id": atom_id,
            "statement": _text(atom.get("statement"), "slice_result.atom.statement"),
            "raw_refs": [evidence_id], "confidence": str(atom.get("confidence", "verified")),
            "execution_profile": profile, "scope_class": scope, "created_at": completed_at,
            ...
            "provenance": [slice_id, evidence_id], "repository": result.get("repository"),
            "unity_version": result.get("unity_version"), "platform": result.get("platform"),
            "tags": list(result.get("tags", [])) if isinstance(result.get("tags", []), list) else [],
        })
        if atom_projection.get("status") != "ok":
            return _envelope("finalize", "blocked", ready=False, evidence=capture, atom=atom_projection, required_next_action="resolve_memory_atom")
        memory_records.append(atom_id)

    if previous.get("quota_spent") is True and previous.get("slice_id") == slice_id:
        return _envelope(
            "finalize", "ok", ready=True, evidence=capture, atom=atom_projection,
            quota={"status": "ok", "operation": "spend", "spent_delta": 0, "idempotent_replay": True, "quota": state.get("quota", {})},
            required_state_writeback={},
            diagnostics=[{"code": "finalize_idempotent_replay", "message": "Quota was already spent for this slice."}],
        )
    if previous.get("quota_spent") is True and previous.get("slice_id") != slice_id:
        return _envelope(
            "finalize", "blocked", ready=False, evidence=capture, atom=atom_projection,
            required_next_action="reprepare_from_authoritative_state",
            diagnostics=[{"code": "quota_accounting_conflict", "message": "A different slice is already accounted; new evidence was preserved."}],
        )
    if not state_matches_ticket:
        return _envelope(
            "finalize", "blocked", ready=False, evidence=capture, atom=atom_projection,
            required_next_action="reprepare_from_authoritative_state",
            diagnostics=[{"code": "stale_execution_state", "message": "State changed after ticket issuance; evidence was preserved and accounting blocked."}],
        )

    spend_state = copy.deepcopy(state)
    spend_previous = dict(spend_state.get("previous_slice") or {})
    spend_previous.update({
        "slice_id": slice_id, "writeback_complete": True, "validated": True,
    ...
    if spend.get("status") != "ok":
        return _envelope(
            "finalize", "blocked", ready=False, evidence=capture, atom=atom_projection, quota=spend,
            required_next_action="resolve_quota_accounting",
            diagnostics=[{"code": "evidence_preserved_without_quota_spend", "message": "Evidence is durable but quota spend was rejected."}],
        )

    spend_id = f"spend-{_digest({'ticket': ticket.get('ticket_id'), 'slice': slice_id})[:24]}"
    state_patch = {
        "previous_slice": {
            "slice_id": slice_id, "todo_id": todo_id, "writeback_complete": True, "validated": True,
            "evidence_refs": spend_previous["evidence_refs"], "quota_spent": True,
            "quota_spend_id": spend_id, "orchestrated": True,
        },
        "quota": {"spent_slots": (spend.get("quota") or {}).get("spent_slots")},
        "orchestration": {
            "active_ticket_id": None, "last_quota_spend_id": spend_id,
            "required_next_action": "write_finalization_to_authoritative_state", "last_status": "ok",
        },
        "memory_projection": {"memory_ids": memory_records, "raw_evidence_refs": [evidence_id]},
    }
    return _envelope(
        "finalize", "ok", ready=True, evidence=capture, atom=atom_projection, quota=spend,
        required_state_writeback=state_patch, required_next_action="write_finalization_to_authoritative_state",
    )


def _envelope(
    operation: str, status: str, *, ready: bool, decision=None, claim=None, memory=None,
    code_intelligence=None, source_verification=None, ticket=None, evidence=None, atom=None,
    quota=None, required_state_writeback=None, required_next_action=None, diagnostics=None,
) -> dict[str, Any]:
    return {
        "schema_version": SCHEMA_VERSION,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = _envelope(args.operation, "invalid_request", ready=False, diagnostics=[{"code": "invalid_input", "message": str(exc)}])
    except OrchestrationError as exc:
        result = _envelope(args.operation, exc.status, ready=False, diagnostics=[{"code": exc.code, "message": exc.message}])
